...
class Node:
    ...
    def path(self):
        ...
        while x.parent:
            ...
            x = x.parent
        result.reverse() #reverse ordering
	return result
...
def graph_search(problem, fringe):
	...
	node_count = node_count + 1
        if problem.goal_test(node.state): 
            return node,max_nodes_in_memory,node_count
        if node.state not in closed:
            closed[node.state] = True
            #Use a set for better implementation?
            fringe.extend(node.expand(problem))    
    return None,max_nodes_in_memory,node_count
